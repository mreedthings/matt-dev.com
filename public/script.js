...
function wrapTextNode(node) {
    ...
    const isLogo = !!(node.parentElement && node.parentElement.closest('#logo'));
    const isLink = !!(node.parentElement && node.parentElement.closest('a'));
    ...
    for (const char of text) {
        ...
        const rotation = isLogo ? randomBetween(-1.5, 1.5) : randomBetween(-0.75, 0.75);
        const offsetY = isLogo ? randomBetween(-1, 1) : randomBetween(-0.5, 0.5);
        const letterSpacing = isLogo ? randomBetween(-0.3, 0.3) : randomBetween(-0.15, 0.15);
        ...
        if (isLogo) {
            ...
            const saturation = randomBetween(60, 75);
            const lightness = randomBetween(60, 70);
        ...
        if (isLink) {
            ...
            const saturation = randomBetween(60, 75);
            const lightness = randomBetween(60, 70);
            span.style.setProperty('--rainbow-color', `hsl(${hue}, ${saturation}%, ${lightness}%)`);
